...
TrackSettings settings_from_dict(const py::dict &dict) {
  ...
  update(dict, "num_successes_before_increase_",
         settings.num_successes_before_increase_);
...
py::dict
radial_quadrifocal_solver_wrapper(const std::vector<Eigen::Vector2d> &x1,
  ...
  int valid = radial_quadrifocal_solver(x1, x2, x3, x4, start_system,
                                        track_settings, P1, P2, P3, P4, QFs);
...
py::dict metric_upgrade_wrapper(const std::vector<Eigen::Vector2d> &x1,
  ...
  std::vector<Eigen::Matrix<double, 2, 4>> P1_calib, P2_calib, P3_calib,
      P4_calib;
  std::vector<std::vector<Eigen::Vector3d>> Xs;

  int valid = metric_upgrade(x1, x2, x3, x4, P1, P2, P3, P4, P1_calib, P2_calib,
                             P3_calib, P4_calib, Xs);
...
void generate_start_system_code(const std::string &filename) {
  ...
  for (int i = 0; i < n; ++i) {
    std::cout << "{" << start_system.problem[i].real() << ","
              << start_system.problem[i].imag() << "}";
    if (i < n - 1) {
      std::cout << ",";
    } else {
      std::cout << "};\n";
  ...
  for (int i = 0; i < n1; ++i) {
    std::cout << "{";
    for (int j = 0; j < n2; ++j) {
      std::cout << "{" << start_system.sols[i][j].real() << ","
                << start_system.sols[i][j].imag() << "}";
      if (j < n2 - 1) {
        std::cout << ",";
      }
    }
...
PYBIND11_MODULE(pyrqt, m) {
  ...
  */
  ...
  m.def("radial_quadrifocal_solver", &radial_quadrifocal_solver_wrapper,
        py::arg("x1"), py::arg("x2"), py::arg("x3"), py::arg("x4"),
        py::arg("track_settings"),
        "Minimal solver for radial quadrifocal tensor",
        py::call_guard<py::scoped_ostream_redirect,
                       py::scoped_estream_redirect>());

  m.def("metric_upgrade", &metric_upgrade_wrapper, py::arg("x1"), py::arg("x2"),
        py::arg("x3"), py::arg("x4"), py::arg("P1"), py::arg("P2"),
        py::arg("P3"), py::arg("P4"),
        "Upgrades a projective reconstruction to metric.",
        py::call_guard<py::scoped_ostream_redirect,
                       py::scoped_estream_redirect>());

  m.def("generate_start_system_code", &generate_start_system_code,
        py::arg("filename"),
