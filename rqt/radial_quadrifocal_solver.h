...
// tensors in QFs
int radial_quadrifocal_solver(const std::vector<Eigen::Vector2d> &p1s,
                              const std::vector<Eigen::Vector2d> &p2s,
                              const std::vector<Eigen::Vector2d> &p3s,
                              const std::vector<Eigen::Vector2d> &p4s,
...
bool load_settings(
    std::string set_file, // location of the file with the settings
    struct TrackSettings
        &settings); // OUTPUT: variable where the settings is stored
...
bool load_start_system(
    ...
    std::vector<std::complex<double>>
        &problem, // OUTPUT: coefficients of the starting problem
    std::vector<std::vector<std::complex<double>>>
        &sols); // OUTPUT: 28 solutions to the starting problem
