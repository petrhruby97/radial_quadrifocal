...
int filter_cheirality(const std::vector<Eigen::Vector2d> &x1,
  ...
  for (int k = 0; k < 13; ++k) {
    ...
    if (k == 0) {
      ...
      if (s1 * alpha1 < 0 || s2 * alpha2 < 0 || s3 * alpha3 < 0 ||
          s4 * alpha4 < 0) {
        return 0;
      }
    }
...
int metric_upgrade(const std::vector<Eigen::Vector2d> &x1,
                   const std::vector<Eigen::Vector2d> &x2,
                   const std::vector<Eigen::Vector2d> &x3,
                   const std::vector<Eigen::Vector2d> &x4,
                   const Eigen::Matrix<double, 2, 4> &P1,
  ...
  Eigen::JacobiSVD<Eigen::Matrix<double, 8, 10>> svd(
  ...
  for (int q = 0; q < alphas.size(); ++q) {
    ...
    Eigen::Vector3d r13 = P1c.block<1, 3>(0, 0).transpose().cross(
        P1c.block<1, 3>(1, 0).transpose());
    r13 = r13 / r13.norm();
    Eigen::Matrix3d R1;
    ...
    double t12 =
        (P2c(1, 3) - R2f(1, 0) * P1c(0, 3) - R2f(1, 1) * P1c(1, 3)) / R2f(1, 2);
    ...
    */
    ...
    int valid = filter_cheirality(x1, x2, x3, x4, P1f, P2f, P3f, P4f, P1_calib,
                                  P2_calib, P3_calib, P4_calib, Xs);
