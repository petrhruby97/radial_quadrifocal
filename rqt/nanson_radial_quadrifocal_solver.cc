...
void nanson_sol2cam(Eigen::Matrix<double,16,1> QF, Eigen::Matrix<double, 2, 4> *Ps)
{
	double sc = QF(0);
                              ...
                              std::vector<Eigen::Matrix<double, 16, 1>> &QFs)
{
	...
	{		
		...
		//track the problem
		int status = nanson::track(settings, cur_start, params, solution, &num_steps);
		if(status==2)
		{
